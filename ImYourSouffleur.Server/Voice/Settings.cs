...
namespace ImYourSouffleur.Server.Voice
{
    public class Settings
    {
        ...
        private static readonly string EmbeddedTTSpeechModelLicense = "This model and the software may not be used or distributed in any manner except as authorized under a valid written agreement, using the reference number 2648158. The model and software are licensed and not sold, and the agreement provides limited rights to use the model and the software and Microsoft reserves all other rights. You may not work around any technical limitations in the model or the software, reverse engineer, decompile, or disassemble the model or the software; remove, minimize, block or modify any notices of Microsoft or its suppliers in the model or the software; or, share, publish, rent, or lease the model or software, or provide the model or software as a standalone solution for others to use."; // or set EMBEDDED_SPEECH_MODEL_LICENSE

        private static readonly string EmbeddedSpeechTTModelLicense = "This model and the software may not be used or distributed in any manner except as authorized under a valid written agreement, using the reference number 2648157. The model and software are licensed and not sold, and the agreement provides limited rights to use the model and the software and Microsoft reserves all other rights. You may not work around any technical limitations in the model or the software, reverse engineer, decompile, or disassemble the model or the software; remove, minimize, block or modify any notices of Microsoft or its suppliers in the model or the software; or, share, publish, rent, or lease the model or software, or provide the model or software as a standalone solution for others to use.";
        ...
        private static readonly string EmbeddedSpeechRecognitionModelPath = @"C:\tool\STTModels"; // or set EMBEDDED_SPEECH_RECOGNITION_MODEL_PATH
        ...
        public static string EmbeddedSpeechRecognitionModelName = "fr-FR"; // or set EMBEDDED_SPEECH_RECOGNITION_MODEL_NAME
        ...
        // not inside an archive, and they must be readable by the application process.
        private static readonly string EmbeddedSpeechSynthesisVoicePath = @"c:\tool\TTSModels"; // or set EMBEDDED_SPEECH_SYNTHESIS_VOICE_PATH
        ...
        private static readonly string EmbeddedSpeechSynthesisVoiceName = "en-US-JennyNeural"; // or set EMBEDDED_SPEECH_SYNTHESIS_VOICE_NAME
        ...
        private static string SpeechModelSTTLicense;
        private static string SpeechModelTTSLicense;
        ...
        public static EmbeddedSpeechConfig CreateEmbeddedSpeechConfig()
        {
            ...
            */
            ...
            if (!string.IsNullOrEmpty(SpeechRecognitionModelName))
            {
                ...
                config.SetSpeechRecognitionModel(SpeechRecognitionModelName, SpeechModelSTTLicense);
            }

            if (!string.IsNullOrEmpty(SpeechSynthesisVoiceName))
            {
                // Mandatory configuration for embedded speech synthesis.
                config.SetSpeechSynthesisVoice(SpeechSynthesisVoiceName, SpeechModelTTSLicense);
            ...
            if (!string.IsNullOrEmpty(SpeechTranslationModelName))
            {
                ...
                config.SetSpeechTranslationModel(SpeechTranslationModelName, SpeechModelTTSLicense);
            }
        ...
        public static async Task<bool> VerifySettingsAsync()
        {
            ...
            SpeechModelSTTLicense = GetSetting("EMBEDDED_SPEECH_TT_MODEL_LICENSE", EmbeddedSpeechTTModelLicense);
            if (string.IsNullOrEmpty(SpeechModelSTTLicense) || SpeechModelSTTLicense.Equals("YourEmbeddedSpeechModelLicense"))
            {
                Console.Error.WriteLine("## ERROR: The embedded speech model license is not set.");
                return false;
            }

            SpeechModelTTSLicense = GetSetting("EMBEDDED_TT_SPEECH_MODEL_LICENSE", EmbeddedTTSpeechModelLicense);
            if (string.IsNullOrEmpty(SpeechModelTTSLicense) || SpeechModelTTSLicense.Equals("YourEmbeddedSpeechModelLicense"))
            {
                Console.Error.WriteLine("## ERROR: The embedded speech model license is not set.");
                return false;
            }
