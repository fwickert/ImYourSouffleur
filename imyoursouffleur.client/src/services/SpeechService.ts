...
export async function getMicroRecognitionAsync(lang: string) {
    lang = "fr-FR";
    const response = await fetch(`/api/speech/micro?lang=${encodeURIComponent(lang)}`, {
        method: 'GET',
        headers: {
    ...
    if (!response.ok) {
        throw new Error('Failed to call /api/speech/micro endpoint');
    }
